...
const dataDetail = ref({
  ...
  kepala: getIdentitas.value.kepala,
  alamat: getIdentitas.value.alamat,
  telp: getIdentitas.value.telp,
  status_login: getIdentitas.value.status_login
    ? getIdentitas.value.status_login
    : "Aktif",
});
...
const doStoreData = async (d) => {
  let dataStore = {
    ...
    kepala: dataDetail.value.kepala,
    alamat: dataDetail.value.alamat,
    telp: dataDetail.value.telp,
    status_login: dataDetail.value.status_login
      ? dataDetail.value.status_login
      : "Aktif",
  };
  try {
    const response = await Api.post(`yayasan/myprofile/update`, dataStore);
...
const doStoreDataForm2 = async (d) => {
  ...
  try {
    const response = await Api.post(
      `yayasan/myprofile/updatepassword`,
      dataStore
    );
    Toast.success("Success", "Data Berhasil update!");
    // router.go();
    ...
    >
      <div class="card xl:w-2/5 bg-base-100 shadow-xl">
        ...
        <div class="px-10 pt-2">
          <p class="text-sm font-bold">Pilih Logo Yayasan :</p>
          <input
            type="file"
      ...
      <div class="card xl:w-2/5 bg-base-100 shadow-xl">
        ...
        <div class="card-body items-center text-center">
          <h2 class="card-title">{{ getSekolah.kepala }}</h2>
          <p>KEPALA YAYASAN</p>
          <div class="card-actions">
        ...
        <div class="px-10 pt-2">
          <p class="text-sm font-bold">Pilih Photo Kepala YAYASAN :</p>
          <input
            type="file"
